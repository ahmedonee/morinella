...
//		$shop_id = 0;
    ...
    {
        $allow_load_cache = true;
        if (Tools::getValue('submitNewsletter') !== false && Tools::getValue('email') != false) {
            # Newsletter
            $allow_load_cache = false;
        }
        ...
        if (!$this->isCached($tplFile, $cache_id) || !$allow_load_cache) {
            # generate cache
            if (!$this->_widgets) {
...
//			$old_context_shop_id = $context->shop->getContextShopID();
    ...
    {
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'hook_module` hm
    ...
    {
        return $this->_processHook('displayFooter').$this->header_content;
    }
    ...
    public function hookHeader()
    {
        ...
        $this->header_content = $this->display(__FILE__, 'views/widgets/header.tpl');
    }
    # product hook list
